...
var isFixed = function isFixed(elem) {
  var currentElement = elem;
  var fixed = false;

  while (currentElement) {
    var _getComputedStyle, _currentElement;

    fixed = ((_getComputedStyle = getComputedStyle(currentElement)) == null ? void 0 : _getComputedStyle.position) === 'fixed'; // ignore fixed elements for body and above.
    ...
    currentElement = (_currentElement = currentElement) == null ? void 0 : _currentElement.parentElement;
  }
...
function getViewRect(element) {
  var _getElementLayout2 = getElementLayout(element),
      ...
      offsetLeft = _getElementLayout2.offsetLeft; // make sure there are no duplicate elements here.


  var scrollableAncestors = Array.from(new Set(getScrollableAncestors(element.parentNode)));
...
var DndContext = /*#__PURE__*/memo(function DndContext(_ref) {
  ...
  var scrollDetectionCoordinates = activeNodeRect ? add(centerOfRectangle(activeNodeRect, activeNodeRect.left, activeNodeRect.top), translate) : defaultCoordinates;
